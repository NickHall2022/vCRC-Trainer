...
export function StripsWindow() {
  ...
  return (
    <div
      className="preventSelect container"
      style={{
        position: 'relative',
        height: '100vh',
        width: '570px',
    ...
    >
      <div style={{ zIndex: '2', position: 'relative', width: '550px' }}>
        <Grid
          container
          className="stripsBar"
          style={{ backgroundColor: '#1a1a1a', padding: '5px' }}
          textAlign={'center'}
        >
          <Grid size={'grow'}>
            <button
              className={`stripsBarButton${selectedBay === 'ground' ? ' selected' : ''}`}
              onClick={() => handleBayButtonClicked('ground')}
              onDrop={() => handleBayButtonDropped('ground')}
          ...
          <Grid size={'grow'}>
            <button
              className={`stripsBarButton${selectedBay === 'local' ? ' selected' : ''}`}
              onClick={() => handleBayButtonClicked('local')}
              onDrop={() => handleBayButtonDropped('local')}
              onDragOver={handleBarDragOver}
            >
              LC
            </button>
          ...
          <Grid size={'grow'}>
            <button
              className={`stripsBarButton${selectedBay === 'spare' ? ' selected' : ''}`}
              onClick={() => handleBayButtonClicked('spare')}
              onDrop={() => handleBayButtonDropped('spare')}
              onDragOver={handleBarDragOver}
            >
              SPARE
            </button>
          ...
          <Grid size={'grow'}>
            <button
              className="stripsBarPrintButton"
              onDrop={() => deleteStrip(draggedStrip.id)}
              onDragOver={handleBarDragOver}
              style={{
                marginRight: '8px',
                padding: '10px',
            ...
            <button
              className="stripsBarPrintButton"
              onClick={() => handleBayButtonClicked('printer')}
              style={{
                padding: '10px',
                paddingTop: '5px',
                paddingBottom: '5px',
      ...
      <StripsBay
        handleDrop={() => moveStripToBay(draggedStrip, selectedBay)}
